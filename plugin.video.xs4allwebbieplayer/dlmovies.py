...
def download_vod(forceUpdate=False):
    try:
        if forceUpdate == False:
            ...
            fileCache = files.openFile(path.addonstoragecache('moviesvod.js'))
            if fileCache != None:
        ...
        files.saveFile(path.addonstoragecache('moviesvod.js'), JsonDumpBytes)
...
def download_program(forceUpdate=False):
    try:
        if forceUpdate == False:
            ...
            #Check if already cached in files
            fileCache = files.openFile(path.addonstoragecache('moviesprogram.js'))
            if fileCache != None:
        ...
        #Update file cache
        JsonDumpBytes = json.dumps(DownloadDataJson).encode('ascii')
        files.saveFile(path.addonstoragecache('moviesprogram.js'), JsonDumpBytes)
